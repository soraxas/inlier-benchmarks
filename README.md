...
fixed 512-correspondence pair sampled across confidence levels. Full runs
stratify selected pairs across each scene's correspondence-count range instead
of selecting only the largest/easiest match sets. It does not use source images.
...
essential fitting, tutorial homographies, and EPOS `solvePnPRansac` absolute
pose. It uses the same confidence/iteration profiles and RNG seeds, and
measures only the OpenCV robust-estimation call. Its pose and
